...
#Создаем окошко входа под своим аккаунтом
root1=Tk()
root1.title('Login')
...
#Тут вводим ФИО, выпадающее из списка, функцию можно посмотреть по документации
lbl = Label(Frame1, font=('font', 12, 'bold'), text="Ваше имя",  bd=10)
lbl.grid(row=0, column=0)
btn2 = ttk.Combobox(Frame1, font=('font', 12, 'bold'),width=32) #textvariable=Prove
btn2['value'] = DatabaseForm.DataEntry2()
btn2.current()
btn2.grid(row=0, column=1)

# Блок с паролем
lbl2 = Label(Frame1, font=('font', 12, 'bold'), text="Пароль",  bd=10)
...
def zz():
    ...
    # передаем параметрами имя и пароль к функции, и если есть такой пользователь, вернется число больше 1
    # Соответсвенно число больше 1 является условием
    if DatabaseForm.DataEntryLog(name,passw)>0:
        root1.destroy() # рушим окнро с паролем и логином
        root = Tk()
        root.iconbitmap("unnamed.ico") # Можно убрать если не нужна иконка
        root.title("Наработки| Внесение данных")
        #root.attributes('-fullscreen',True)
        root.geometry("1070x653")
        root.configure(background="gainsboro")

        MainFrame = Frame(root, bd=10, width=1300, height=1300, relief=RIDGE,bg='BLACK')
        MainFrame.pack_propagate(0)
        MainFrame.pack(fill='both', side='left', expand='True')
        MainFrame.grid()
        TopFrame1 = Frame(MainFrame, bd=10, width=100, height=200, relief=RIDGE,bg='blue')
        ...
        innerTopFrame3.grid()
        #innerTopFrame4 = Frame(TopFrame3, bd=5, width=1330, height=30, relief=RIDGE)
        #innerTopFrame4.grid()
        ...
        lblstrdel= Label(TopFrame4, font=('font', 12, 'bold'), text="                                               Введите номер строки для удаления:", bd=10)
        lblstrdel.grid(row=0,column=0)
        txtstrdel = Entry(TopFrame4, font=('font', 12, 'bold'), bd=5, width=32, justify='left')
        ...
        txtReference = Entry(innerTopFrame1, font=('font', 12, 'bold'), bd=5, width=32, justify='left')
        ...
        txtSumofPay = Entry(innerTopFrame1, font=('font', 12, 'bold'), bd=5, width=32, justify='left')
        ...
        txtAdress = Entry(innerTopFrame1, font=('font', 12, 'bold'), bd=5, width=95, justify='left')
        ...
        def iExit(): # Выход из программы
            ...
            if iExit > 0:
                root.destroy()
                return
        # Блок обновления таблицы в innerTopFrame3, при внесении новой строки
        def sqlinput():
            if len (cbostatus.get())!=0:
                tree.delete(*tree.get_children())
                DatabaseForm.sqlinto(name,cbogroup.get(),cbooperator.get(),txtReference.get(), txtSumofPay.get(), datetoday,cboRegistrationDate.get(),cbostatus.get(),txtAdress.get())
        ...
        # Удаляет по айди строки
        def delete_id():
            DatabaseForm.del_string(txtstrdel.get())
            tree2 = ttk.Treeview(innerTopFrame3, height=10, columns=3)
            dataf2 = DatabaseForm.sqloutof(name)
            tree2["column"] = list(dataf2.columns)
            tree2["show"] = "headings"
        ...
        btnAddNew = Button(innerTopFrame2, pady=1, bd=4, fg='Black', font=('font', 16, 'bold'), width=18, height=1,text="На всякий случай").grid(row=0, column=1, padx=3)#Тут была команда экзекер
        btnAddNew = Button(innerTopFrame2, pady=1, bd=4, fg='black', font=('font', 16, 'bold'), width=18, height=1,text="Очистить все",command= Reset).grid(row=0, column=2, padx=3)
        ...
        btnAddNew = Button(TopFrame4, pady=1, bd=4, fg='black', font=('font', 16, 'bold'), width=15, height=1,text="Удалить",command= delete_id).grid(row=0, column=2, padx=3)


        root.mainloop()


btnAddNew = Button(Frame1,pady=1,bd=4,fg='black',font=('font',16,'bold'),width=15,height=1,text="Войти",command=zz).grid(row=3,column=0,padx=3)
Button(Frame1,width=35,pady=77,text="go",bg='#57a1f8',fg='black',border=0).place(x=35,y=200)
